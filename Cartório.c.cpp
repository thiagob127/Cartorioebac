...
int registro(){
	...
	fprintf(file,"\nCPF: %s",cpf); //salva o vqlor da variÃ¡vel
	...
	fprintf(file,"\nnome: %s ",nome); //uma forma de salvar no arquivo, para quando ser colsultado nÃ£o ficar com a informaÃ§Ã£o jogada
	fclose(file);
	...
	fprintf(file,"\ncargo: %s",cargo);
	fclose(file);
	system("pause");//muito importante para nâo dar erro
}
...
int deletar(){
	...
	if(file==NULL){//validaçãoo do cpf se está ou nâo no sistema
